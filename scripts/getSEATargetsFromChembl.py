...
SEA_predicted_filtered=pd.read_csv('SEA_predicted_activities_filtered.csv', sep=";")
target_ids_SEA="".join(SEA_predicted_filtered["Target ID"].drop_duplicates().apply(lambda x: "'" + x + "',").to_list())[:-1]
query_SEA="select distinct chembl_31.component_sequences.component_id, chembl_31.component_sequences.accession, chembl_31.component_sequences.organism, chembl_31.component_sequences.tax_id, chembl_31.protein_family_classification.*, chembl_31.organism_class.l1 as 'domain', chembl_31.component_synonyms.component_synonym  from chembl_31.component_sequences"\
...
+" where chembl_31.component_synonyms.component_synonym in ("+ target_ids_SEA+")"\
...
try:
    ...
    if connection.is_connected():
        ...
        unitProtId = pd.read_sql(query_SEA, con=connection)
        unitProtId.to_csv('target_uniprot_ids_sea.csv', sep=";", index=False)
...
sea_target_class=pd.read_csv('target_uniprot_ids_sea.csv', sep=";")
sea_target_class["tcl"] = sea_target_class.apply(lambda x: x.l2 if x.l2 in ["Transferase","Oxidoreductase","Protease",
