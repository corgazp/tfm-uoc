...
pubchem_predicted_filtered=pd.read_csv('pubchem_targets_uniprot_id.csv', sep=";")
target_ids_pubchem="".join(pubchem_predicted_filtered["accesion"].drop_duplicates().apply(lambda x: "'" + x + "',").to_list())[:-1]
query_pubchem="select distinct chembl_31.component_sequences.component_id, chembl_31.component_sequences.accession, chembl_31.component_sequences.organism,chembl_31.component_sequences.tax_id, chembl_31.protein_family_classification.* ,chembl_31.organism_class.l1  from chembl_31.component_sequences"\
...
+" where chembl_31.component_sequences.accession in ("+ target_ids_pubchem +")"\
...
try:
    ...
    if connection.is_connected():
        ...
        unitProtId_pubchem = pd.read_sql(query_pubchem, con=connection)
        unitProtId_pubchem.to_csv('target_uniprot_ids_pubchem.csv', sep=";", index=False)
...
pubchem_target_class=pd.read_csv('target_uniprot_ids_pubchem.csv', sep=";")
pubchem_target_class["tcl"] = pubchem_target_class.apply(lambda x: x.l2 if x.l2 in ["Transferase","Oxidoreductase","Protease",
